...
class Buynow extends Component
{
    ...
    public $subtotal = 0, $total_weight, $description, $cost_value, $temp_courier;
    public $etd = "0";
    ...
    public function render()
    {
        ...
        if (!is_null($this->input_provinsi)) {
            $temp_provinsi = Province::where('title', '=', $this->input_provinsi)->first();
            ...
            if (!is_null($this->input_kurir) && !is_null($this->input_region)) {
                ...
                $this->total_weight =  $this->product->weight * $this->qty;

                $temp_courier = Courier::find($this->input_kurir);
                $getCost = $client->request(
                    'POST',
                    $url,
                    [
                        'headers' => [
